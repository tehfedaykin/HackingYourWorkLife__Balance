...
function titleBlink(ele, wordIdx, words) {
  return setInterval(() => {  
    var newWord = '_' + words[wordIdx] + '_';
    ele.text(newWord, wordIdx);
    wordIdx = wordIdx < (words.length - 1) ? ++wordIdx : 0;
    wordBlink = blinkFade(ele);
  }, 5000);
}
...
Reveal.addEventListener('ready', function (ev) {
    ...
    var wordIdx = 0;
    var words = ['','passion', 'volunteer', 'speaking', 'open source', 'competitve dancing', 'side hustle', 'blank']
    blinkingTitle = titleBlink(blankWord, wordIdx, words);
});
Reveal.addEventListener('slidechanged', function( event ) {
  window.clearInterval(wordBlink);
  window.clearInterval(blinkingTitle);
